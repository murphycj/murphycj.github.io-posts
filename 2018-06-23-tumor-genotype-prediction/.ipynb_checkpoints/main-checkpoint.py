...
def pca_analysis(sample_info):
    ...
    X = pd.read_csv('./data/genes-symbols-fpkm.csv',index_col=0)
...
# format data into datafram

exon_counts = pd.read_csv('./data/method1/brca1.exon_counts.rnaseq.csv')
...
scatter_matrix(exon_counts.T.loc[:,0:12],figsize=(12,12))
...
p = pn.ggplot(data,pn.aes('exon','normalized_read_count',color='brca1')) + \
    pn.geom_boxplot() + \
    pn.theme(
        axis_text_x=pn.element_text(rotation=90, hjust=1,size=12),
